...
export const veriEmail = async (req, res) => {
  try {
    ...
    if (busqueda.verification) {
      return res
        .status(400)
        .json({ msg: "the email not required verification" });
    }
    if (!busqueda.verification) {
      ...
      let button = {
        ...
        link: `http://${url2}/api/v1.0/verify/tk/${token}`,
      };
      let info =
        "Te has registrado exitosamente en adoptaMe, por favor confirma tu correo abajo";
...
export const petiPass = async (req, res) => {
  try {
    ...
    if (!busqueda) {
      res.status(400).json({ msg: "the email is not registered" });
    }
    ...
    let button = {
      ...
      link: `http://${url2}/api/v1.0/verify/modpass/${token}`,
    };
    let info =
      "has solicitado una recuperacion de contraseña, si no lo hiciste ignora este mensaje";
